...
def fcn_Minkowski(xx, yy, mu, p=2, Chebychev=False):
    if Chebychev:
        ...
        zz = ((np.abs((xx - mu[0])) ** p) + (np.abs((yy - mu[1])) ** p)) ** (1.0 / p)
...
fig_2.add_trace(
    go.Contour(x=x_array, y=y_array, z=zz, contours_coloring="lines", showscale=False)
)
...
fig_2.add_traces(
    px.scatter(X.mean().to_frame().T, x="sepal_length", y="petal_length")
    .update_traces(marker_size=20, marker_color="red", marker_symbol="x")
    .data
)
